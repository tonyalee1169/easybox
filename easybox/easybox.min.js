...
(function(a){function Q(d){d?a("object:visible").add("embed").each(function(b,a){F[b]=[a,a.style.visibility];a.style.visibility="hidden"}):(a.each(F,function(b,a){a[0].style.visibility=a[1]}),F=[]);d=d?"bind":"unbind";a(document)[d]("keydown",Z);if(a.fn.mousewheel)a(window)[d]("mousewheel",$);a(f)[d]("mousedown",aa)[d]("mousemove",R)[d]("mouseup",G);a(window)[d]("mousemove",R)[d]("mouseup",G)}function Z(d){var d=d.keyCode,b=a.inArray;return u?!0:0<=b(d,c.closeKeys)?B():0<=b(d,c.nextKeys)&&!c.noNavigation?
v():0<=b(d,c.previousKeys)&&!c.noNavigation?w():!c.preventOtherKeys}function $(a,b){return u?!0:0<b&&!c.noNavigation?w():0>b&&!c.noNavigation?v():!c.preventOtherKeys}function w(){o=!0;return H(i)}function v(){o=!1;return H(g)}function S(){if(c.slideshow&&!p&&null==k){if(o&&0<=i)return k=setTimeout(w,c.slideshow),!1;if(!o&&0<=g)return k=setTimeout(v,c.slideshow),!1}c.autoClose&&null==x&&(x=setTimeout(T,c.autoClose));return!1}function H(d){0<=d&&d<e.length&&(j=d,i=(j||(c.loop?e.length:0))-1,g=(j+1)%
e.length||(c.loop?0:-1),I(),c.busyTimeout&&(C=setTimeout(ba,c.busyTimeout)),c.preloadNext&&(0<=i&&J(i),0<=g&&J(g)),e[j].loaded||a(K).show(),J(j));return!1}function ca(){var d=e[j];a(K).hide();if(d.error)a(h).addClass("error").width(c.errorSize[0]).height(c.errorSize[1]);else{if("inline"==d.type)q=d.obj,L=a(q).parent(),M=a(q).css("display");d.caption.length&&a(y).html(d.caption).css({display:""});a(h).width(d.width).height(d.height)}1<e.length&&c.counterText.length&&a(z).html(c.counterText.replace(/{x}/,
...
!1;if(!c.hideBottom){if((0<=i||0<=g)&&!c.noNavigation&&!c.hideButtons)a(N).css({display:""}),a([y,z]).addClass("nav"),0<=i&&a(r).fadeIn(c.captionFadeDuration),0<=g&&a(s).fadeIn(c.captionFadeDuration);a(A).fadeIn(c.captionFadeDuration);a(f).animate({height:l[1]+A.offsetHeight},c.captionFadeDuration,c.resizeEasing)}}function I(){u=!0;null!=k&&(clearTimeout(k),k=null);null!=C&&(clearTimeout(C),C=null);null!=x&&(clearTimeout(x),x=null);null!=q&&(a(L).append(a(q).css({display:M})),q=L=M=null);a(h).empty().removeClass();
a([f,A,h,r,s]).stop(!0);a(f).css({width:l[0],height:l[1],marginLeft:-l[0]/2,marginTop:-l[1]/2,opacity:""});a([N,y,z,h,A,r,s]).css({display:"none",opacity:""});a([y,z]).removeClass().html("").css({display:c.hideCaption?"none":""})}function ea(){p=!p;o=!1;a(O).toggleClass("disabled",p);p?null!=k&&(clearTimeout(k),k=null):S();return!1}function B(d){if(!(c.noClose&&1!=d))return 0<=j&&(I(),j=i=g=-1,a(P).stop().fadeOut(c.fadeDuration,Q),U(c.initCenterSize,0,c.fadeDuration),a(f).queue(function(b){G();a(f).css({left:"",
top:""}).hide();b()})),!1}function T(){B(1)}function ba(){u=!1}function J(d){var b=e[d];b.loaded&&n(d);if(!b.loading)if(b.loading=!0,/(\.jpg|\.jpeg|\.png|\.gif)$/i.test(e[d].url))b.type="image",fa(d);else{var c=/^http\:\/\/www\.youtube\.com\/watch\?v=([A-Za-z0-9\-_]*)(&(.*))?$/i.exec(e[d].url);!1!=(id=null!=c?c[1]:!1)?(b.type="youtube",b.id=id,V(d)):(c=/^http\:\/\/vimeo\.com\/([0-9]*)(.*)?$/i.exec(e[d].url),!1!=(id=null!=c?c[1]:!1)?(b.type="vimeo",b.id=id,V(d)):(c=/^(.*)\#([A-Za-z0-9\-_]*)$/i.exec(e[d].url),
