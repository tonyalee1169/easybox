...
80],nextKeys:[39,78]},h);c.dynOpts&&a("#easyOptions").children().each(function(){var b=this;(!a(b).attr("data-rel")||typeof i=="string"&&a(b).attr("data-rel")==i)&&a.each(c,function(g){a(b).attr("data-"+g)&&(c[g]=eval(a(b).attr("data-"+g)))})});typeof b=="string"&&(b=[[b,g]],g=0);if(c.maxScreenFill)c.maxWidth=Math.min(Math.round(screen.width*c.maxScreenFill),c.maxWidth),c.maxHeight=Math.min(Math.round(screen.height*c.maxScreenFill),c.maxHeight);d=b;c.loop=c.loop&&d.length>1;Q?u(g):(j=c.initWidth,
l=c.initHeight,a(f).css({width:j,height:l,marginLeft:-j/2,marginTop:-l/2,opacity:""}),L(1),a(f).show(),a(z).css("opacity",c.overlayOpacity).fadeIn(c.fadeDuration,function(){u(g)}));return!1};a.fn.easybox=function(b,c,d){var c=c||function(a){return[a.href,a.title]},d=d||function(a){return this==a},e=this;return e.unbind("click").click(function(){var f=this,j=0,i,h=0,k;i=a.grep(e,function(a,b){return d.call(f,a,b)});for(k=i.length;h<k;++h)i[h]==f&&(j=h),i[h]=c(i[h],h);return a.easybox(i,j,b,a(f).attr("rel")||
null)})};a.easing.easybox=function(a){return a<0.7?Math.pow(a/0.7,2)*1.2:1.2-Math.sqrt((a-0.7)/(1-0.7))*0.2}})(jQuery);
